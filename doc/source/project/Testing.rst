...
``geoid-toolkit`` uses the ``pytest`` framework to run tests and verify outputs.
Running the test suite requires a `dev installation <../getting_started/Install.html>`_ of the ``geoid-toolkit`` package to include all of the optional dependencies.
...
.. code-block:: bash
    ...
    pytest --cov geoid_toolkit --cov-report=term 

.. code-block:: bash

    pixi run test "--cov ../geoid_toolkit --cov-report=term"
...
* Uploading source and wheel distributions to `PyPI <https://pypi.org/project/geoid-toolkit/>`_ (on releases)
