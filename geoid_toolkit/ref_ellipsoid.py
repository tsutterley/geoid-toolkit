...
CALLING SEQUENCE
	wgs84 = ref_ellipsoid('WGS84',CGS=True)
	a_axis = wgs84['a']
	b_axis = wgs84['b']
...
OPTIONS:
	CGS: output in centimeters, grams, seconds (True/False)
		(default is meters, kilograms, seconds)
...
def ref_ellipsoid(refell, CGS=False):
	...
	if refell.upper() in ('CLK66','NAD27'):
		...
		raise ValueError('Incorrect reference ellipsoid Name: see readme')
	...
	a_axis = a_axis*100.0 if CGS else a_axis
	GM = GM*1.0e6 if CGS else GM
