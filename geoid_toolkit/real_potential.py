...
def real_potential(lat, lon, h, refell, clm, slm, lmax, R, GM, GAUSS=0):
    ...
    for m in range(lmax, -1, -1):
        ...
        ds_m_dr_c[:, 2 * m : 2 * m + 2] = clenshaw_ds_m_dr(
            t, q, m, clm, slm, lmax
        )
    ...
    ds_m_dr = (
        ds_m_dr_c[:, 2 * lmax] * cos_m_phi[:, lmax]
    ...
    for m in range(lmax - 1, 0, -1):
        ...
        a_m = np.sqrt(
            (2.0 * np.longdouble(m) + 3.0) / (2.0 * np.longdouble(m) + 2.0)
        )
        s_m = (
            a_m * u * q * s_m
...
def clenshaw_s_m(t, q, m, clm1, slm1, lmax):
    ...
    if m == lmax:
        ...
        a_lm = (
            np.sqrt(((2.0 * lm - 1.0) * (2.0 * lm + 1.0)) / (lm * lm)) * t * q
        )
...
def clenshaw_ds_m(t, u, q, m, clm1, slm1, lmax):
    ...
    if m == lmax:
        ...
        for l in range(lmax - 2, m - 1, -1):
            ...
            s_dot_mm_c = (
                a_lm * (s_dot_mm_c_pre_1 * t + s_mm_c_pre_1)
                - b_lm * s_dot_mm_c_pre_2
            )
            s_dot_mm_s = (
                a_lm * (s_dot_mm_s_pre_1 * t + s_mm_s_pre_1)
                - b_lm * s_dot_mm_s_pre_2
            )
...
def clenshaw_ds_m_dr(t, q, m, clm1, slm1, lmax):
    ...
    if m == lmax:
        ...
        s_m[:, 0] = (
            a_lm * ds_mm_dr_c_pre_1 * clm[lmax, lmax - 1]
            - lm * clm[lmax - 1, lmax - 1]
        )
        s_m[:, 1] = (
            a_lm * ds_mm_dr_s_pre_1 * slm[lmax, lmax - 1]
            - lm * slm[lmax - 1, lmax - 1]
        )
    elif (m <= (lmax - 2)) and (m >= 1):
        ...
        for l in range(lmax - 2, m - 1, -1):
            ...
            ds_mm_dr_c = (
                a_lm * ds_mm_dr_c_pre_1
                - b_lm * ds_mm_dr_c_pre_2
                - (ll + 1.0) * clm[l, m]
            )
            ds_mm_dr_s = (
                a_lm * ds_mm_dr_s_pre_1
                - b_lm * ds_mm_dr_s_pre_2
                - (ll + 1.0) * slm[l, m]
            )
